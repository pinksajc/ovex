...
export function Simulator({ deal, initialConfig, loadedConfigId }: SimulatorProps) {
  ...
  const hasUnsavedChanges = useMemo(
    () => serializeSimState(dailyOrders, deliveryOrders, locations, avgTicket, planOverride, activeAddons, hardware, renEnabled, renFeePerOrder, renVenues, kdsVenues, kioskVenues, discountPercent, starterIncluded, calculateVariable, discountName, deliveryPlan, itemDiscounts, itemPriceOverrides, whisprPlan, whisprBilling, whisprEnterprisePrice) !== savedSnapshot,
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        ...
        setSavedSnapshot(serializeSimState(dailyOrders, deliveryOrders, locations, avgTicket, planOverride, activeAddons, hardware, renEnabled, renFeePerOrder, renVenues, kdsVenues, kioskVenues, discountPercent, starterIncluded, calculateVariable, discountName, deliveryPlan, itemDiscounts, itemPriceOverrides, whisprPlan, whisprBilling, whisprEnterprisePrice))
        router.refresh()
      } else {
        setSaveState('error')
      }
...
function EconomicsPanel({
  ...
  const adjustedMRR = discountableBase - discountAmount + deliveryFeeEffective + renMonthly + hwMonthlyEffective + whisprMonthly
  const rentedMonthly = HARDWARE_ORDER.reduce((sum, id) => {
    const s = hardware[id]
    return s.mode === 'rented' ? sum + (HARDWARE[id].rentalMonthlyPrice ?? RENTAL_MONTHLY_PRICE) * s.quantity : sum
  }, 0)
