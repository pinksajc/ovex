...
export function ConvertProformaButton({ invoiceId }: { invoiceId: string }) {
  ...
  function handleConvert() {
    startTransition(async () => {
      const result = await convertProformaAction(invoiceId)
      if (!result.ok) {
  ...
  return (
    <>
      ...
      <button
        ...
        disabled={isPending}
        className="w-full text-left px-3 py-2 rounded-lg border border-violet-200 text-violet-700 hover:bg-violet-50 text-xs font-medium transition-colors disabled:opacity-50"
      >
